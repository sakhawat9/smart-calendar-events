...
class Ajax_Function {
	...
	public function __construct() {
		add_action( 'wp_ajax_get_month_events', array( $this, 'get_month_events' ) );
		add_action( 'wp_ajax_nopriv_get_month_events', array( $this, 'get_month_events' ) );
	}
	...
	public function get_month_events() {
		...
		// Nonce verification
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['nonce'] ), 'sce_event_nonce_action' ) ) {
			wp_die( 'Nonce verification failed' );
		}

		$current_month = isset( $_POST['month'] ) ? wp_unslash( $_POST['month'] ) : '';
		$current_year  = isset( $_POST['year'] ) ? wp_unslash( $_POST['year'] ) : '';
